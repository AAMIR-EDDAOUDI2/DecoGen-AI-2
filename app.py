...
# Ensure Javascript and CSS files are served with the correct mime types for Vite
...
def index():
    ...
    return "React build 'dist/index.html' not found. Check build logs.", 404

@app.route('/<path:path>')
def serve_static(path):
    # Check if the exact file exists in the dist folder (like /assets/index.js)
    full_path = os.path.join(app.static_folder, path)
    if os.path.exists(full_path) and os.path.isfile(full_path):
        return send_from_directory(app.static_folder, path)
    
    # If the file isn't found, it's a React Router path, so serve index.html
    if os.path.exists(os.path.join(app.static_folder, 'index.html')):
        return send_from_directory(app.static_folder, 'index.html')
